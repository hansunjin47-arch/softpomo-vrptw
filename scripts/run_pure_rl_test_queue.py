"""run_pure_rl_test_queue.py — Test-only for Pure RL (no-LLM) Config F: C1 -> RC1 -> R1."""
...
LOG_FILE = os.path.join(SOFT, 'logs', 'pure_rl_test_queue.log')
...
QUEUE = [
    ('test_c1',  ['--benchmark', 'c1',  '--config', 'F', '--no-llm', '--test-only', '--result-dir', RESULT_DIR]),
    ('test_rc1', ['--benchmark', 'rc1', '--config', 'F', '--no-llm', '--test-only', '--result-dir', RESULT_DIR]),
    ('test_r1',  ['--benchmark', 'r1',  '--config', 'F', '--no-llm', '--test-only', '--result-dir', RESULT_DIR]),
]

log('=== pure_rl_test_queue started ===')
