"""run_rl_baseonly_queue.py -- Pure RL Config F, base-only training (C1 -> RC1 -> R1).
Used to validate K behavior under standard reward before K_min shaping (Config G).
"""
...
QUEUE = [
    ('rl_baseonly_c1',  ['--benchmark', 'c1',  '--epochs', '1000', '--config', 'F', '--base-only']),
    ('rl_baseonly_rc1', ['--benchmark', 'rc1', '--epochs', '1000', '--config', 'F', '--base-only']),
    ('rl_baseonly_r1',  ['--benchmark', 'r1',  '--epochs', '1000', '--config', 'F', '--base-only']),
]

def _ts():
    return time.strftime('%Y-%m-%d %H:%M:%S')

print(f'[{_ts()}] === Pure RL Config F base-only queue started ===', flush=True)
for tag, args in QUEUE:
    ...
    if r.returncode != 0:
        print(f'[{_ts()}] ERROR: {tag} failed — stopping queue.', flush=True)
        sys.exit(r.returncode)

print(f'[{_ts()}] === Pure RL Config F base-only queue done ===', flush=True)
