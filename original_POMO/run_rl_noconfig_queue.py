"""run_rl_noconfig_queue.py -- no-config base-only: waits for C1, then RC1 -> R1."""
...
C1_CKPT = r'C:\Users\hansu\PycharmProjects\PythonProject\original_POMO\result\c102-c109\checkpoint-last.pt'
os.makedirs(LOG_DIR, exist_ok=True)

QUEUE = [
    ('rl_noconfig_rc1', ['--benchmark', 'rc1', '--epochs', '1000', '--base-only']),
    ('rl_noconfig_r1',  ['--benchmark', 'r1',  '--epochs', '1000', '--base-only']),
]
...
def wait_for_c1():
    print(f'[{_ts()}] Waiting for no-config C1 to finish...', flush=True)
    last_size, stable_count = -1, 0
    while True:
        if os.path.exists(C1_CKPT):
            print(f'[{_ts()}] C1 checkpoint found. Starting RC1.', flush=True)
            return
        c1_log = os.path.join(LOG_DIR, 'rl_noconfig_c1_run.log')
...
print(f'[{_ts()}] === no-config base-only queue (RC1 -> R1) ===', flush=True)
