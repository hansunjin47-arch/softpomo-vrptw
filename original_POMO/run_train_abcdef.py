"""run_train_abcdef.py
6 configs, 3 phases (pairwise):
  Phase 1: A + B  (K>D>Lt, K>Lt>D)
  Phase 2: C + D  (D>K>Lt, D>Lt>K)
  Phase 3: E + F  (Lt>K>D, Lt>D>K)
All: pomo=100, base-only (train c102-c109 / test c101), tag=pomo100
"""
...
GPU_INIT_DELAY = 90   # seconds between launches in a pair
...
print(f'[{_ts()}] === Phase 1: A + B  (K>D>Lt, K>Lt>D) ===', flush=True)
run_pair('A', 'B')

print(f'[{_ts()}] === Phase 2: C + D  (D>K>Lt, D>Lt>K) ===', flush=True)
run_pair('C', 'D')

print(f'[{_ts()}] === Phase 3: E + F  (Lt>K>D, Lt>D>K) ===', flush=True)
run_pair('E', 'F')
