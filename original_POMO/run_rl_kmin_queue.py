"""run_rl_kmin_queue.py -- Pure RL Config G (K_min excess penalty) base-only, RC1 -> R1."""
...
QUEUE = [
    ('rl_kmin_rc1', ['--benchmark', 'rc1', '--epochs', '1000', '--config', 'G', '--base-only']),
    ('rl_kmin_r1',  ['--benchmark', 'r1',  '--epochs', '1000', '--config', 'G', '--base-only']),
]

def _ts():
    return time.strftime('%Y-%m-%d %H:%M:%S')

print(f'[{_ts()}] === Pure RL Config G base-only queue (RC1 -> R1) ===', flush=True)
