"""run_test_allstarts.py
Test all 4 configs (C, D, E, N) with pomo_size=100 (all stops).
Runs 2 at a time: C+D in parallel, then E+N in parallel.
"""
...
DELAY = 30  # seconds between starts in a pair
...
def test_one(config):
    ...
    print(f'[{_ts()}] DONE  Config {config}  {status}  {elapsed:.1f}min  log={log}', flush=True)
...
print(f'[{_ts()}] === Phase 1: Config C + D ===', flush=True)
run_pair('C', 'D')

print(f'[{_ts()}] === Phase 2: Config E + N ===', flush=True)
run_pair('E', 'N')
