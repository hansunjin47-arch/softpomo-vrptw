"""run_rl_config_c_queue.py -- Config C base-only: waits for C1, then RC1 -> R1."""
...
C1_CKPT = r'C:\Users\hansu\PycharmProjects\PythonProject\original_POMO\result\config_C\c102-c109\checkpoint-last.pt'
os.makedirs(LOG_DIR, exist_ok=True)

QUEUE = [
    ('rl_config_c_rc1', ['--benchmark', 'rc1', '--epochs', '1000', '--config', 'C', '--base-only']),
    ('rl_config_c_r1',  ['--benchmark', 'r1',  '--epochs', '1000', '--config', 'C', '--base-only']),
]
...
def wait_for_c1():
    print(f'[{_ts()}] Waiting for C1 Config C to finish...', flush=True)
    last_size, stable_count = -1, 0
    while True:
        if os.path.exists(C1_CKPT):
            print(f'[{_ts()}] C1 checkpoint found.', flush=True)
            return
        c1_log = os.path.join(LOG_DIR, 'rl_config_c_c1_run.log')
...
print(f'[{_ts()}] === Config C base-only queue (RC1 -> R1) ===', flush=True)
